    ...
    for i in range(len(list_of_quantifiers)):
        ...
        if universal_flag:
            if list_of_quantifiers[i].name == "∃":
                ...
                new_predicate = Node(
                    'F' + str(CONSTANT_PREDICATE), children=new_atoms)
                CONSTANT_PREDICATE += 1
                root = replace_atom_with(
                    root, list_of_quantifiers[i].var, new_predicate)
            ...
            if list_of_quantifiers[i].name == "∃":
                ...
                CONSTANT_NO_NAME += 1
                root = replace_atom_with(
                    root, list_of_quantifiers[i].var, new_constant_atom)
...
def unification(p1, p2):
    ...
    for i in range(len(p1.children)):
        if p1.children[i].name == p2.children[i].name:
            continue
        if (p1.children[i].is_constant) == (p2.children[i].is_constant) and not p1.children[i].is_constant:
            continue
        elif (p1.children[i].is_constant) != (p2.children[i].is_constant) and p1.children[i].is_constant:
            continue
        elif (p1.children[i].is_constant) != (p2.children[i].is_constant) and p2.children[i].is_constant:
            continue
        else:
            return False
    ...
    for i in range(len(p1.children)):
        if p1.children[i].name == p2.children[i].name:
            continue
        if (p1.children[i].is_constant) == (p2.children[i].is_constant) and not p1.children[i].is_constant:
            p2.children[i].name = p1.children[i].name
        elif (p1.children[i].is_constant) != (p2.children[i].is_constant) and p1.children[i].is_constant:
            p2.children[i].name = p1.children[i].name
        elif (p1.children[i].is_constant) != (p2.children[i].is_constant) and p2.children[i].is_constant:
            p1.children[i].name = p2.children[i].name
    return True
